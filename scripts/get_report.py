    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                # Se extrae la tabla de la primera página
                table = pdf.pages[0].extract_table()
                if table:
                    for row in table[1:]:
                        if row and row[0] and row[1]:
                            # Limpieza de nombre y ticker
                            ticker = row[0].split('\n')[-1].strip()
                            
                            # Separar Precio y Variación (suelen estar en la misma celda del PDF)
                            parts = row[1].split('\n')
            ...
            print(f"✅ Éxito: {len(stocks)} acciones reales extraídas.")
        else:
            print(f"❌ Error al descargar PDF: Código {response.status_code}")
